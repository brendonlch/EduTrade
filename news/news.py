...
class NewsObj():
       def __init__(self, author, desc, published, ticker, title, url_article): #Initialise the objects
        self.author = author
        self.desc = desc
       ...
       def json(self): 
        return {"author": self.author, "desc": self.desc, "published": self.published, "ticker": self.ticker
            , "title": self.title, "url_article": self.url_article}
...
def get_news_by_tickers():
       ...
       for i in range (1 , len(new_dict) + 1):
              if new_dict[i]['author'] == "":
                     x = NewsObj("No author", new_dict[i]['description'], new_dict[i]['published'], new_dict[i]['ticker'], new_dict[i]['title'], new_dict[i]['url'])
                     news_objects[i] = x.json()
              else:
                     x = NewsObj(new_dict[i]['author'], new_dict[i]['description'], new_dict[i]['published'], new_dict[i]['ticker'], new_dict[i]['title'], new_dict[i]['url'])
                     news_objects[i] = x.json()
       return news_objects
